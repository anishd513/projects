...
int changeop;
int i=0;
float default1[]={91.1,91.9,92.7,93.5,94.3,95,98.3,101.3,104};
float scan[300];
float radio[300];
TouchScreen ts = TouchScreen(XP, YP, XM, YM, 300);
int happen=1;
//TEA5767Radio radio1 = TEA5767Radio();
...
for(int f=0;f<=10;f++)
{scan[f]=0;}
          for(float d=87;d<=104.1;d=d+0.1)
{radio1.setFrequency(i);
delay(100);
//Serial.println(d);
//Serial.println(radio1.getFrequency());
//Serial.print("......");
  // Serial.print(radio1.getSignalLevel());
   //Serial.println();
  
int j=0;
   if(radio1.getSignalLevel()>=5)
{ 
scan[j]=d;
j++;
}
...
tft.reset();
 ...
 tft.println("choose a station");
  if(changeop==2)       
        {
          for(int i=0;i+1<=(sizeof(radio)/sizeof(radio[0]));i++)
  {radio[i]=default1[i];
...
changeop=0;        
 ...
 if (p.z > MINPRESSURE && p.z < MAXPRESSURE) {radio1.setFrequency(radio[i]);
 tft.reset();
